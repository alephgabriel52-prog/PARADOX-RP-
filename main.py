...
async def setup(ctx, fac):
    ...
    if fac not in FAC_LISTA: return await ctx.send("❌ Facs: PM, PC, PRF, PF, SAMU")
    ...
    overwrites = {ctx.guild.default_role: discord.PermissionOverwrite(view_channel=False), ctx.guild.get_role(cargo_ids[0]): discord.PermissionOverwrite(view_channel=True)}
