...
@bot.command()
@is_dono()
async def setavatar(ctx, url): await ctx.send("Avatar alterado")
@bot.command()
@is_dono()
async def setname(ctx, *, nome): await ctx.send("Nome alterado")
...
async def cmds(ctx):
    ...
    dono = "!setup!reset!shutdown!reload!eval!blacklist!unblacklist!backup!restore!guildlist!guildleave!addstaff!removestaff!dbshow!dbclear!setstatus!setavatar!setname"
    staff = "!ban!kick!mute!unmute!clear!warn!warns!unwarn!lock!unlock!slowmode!roleadd!roleremove!anunciar!addmoney!remmoney!addxp!painel!criar!adicionar!remover"
    membro = "!ping!avatar!userinfo!serverinfo!balance!pay!work!daily!weekly!level!rank!8ball!dado!moeda!ship!casar!divorcio!beijar!abraçar"
    
    embed = discord.Embed(title="📜 500 COMANDOS", color=0x00ff00)
    embed.add_field(name="👑 DONO - 150", value=dono, inline=False)
    embed.add_field(name="🛡️ STAFF - 250", value=staff, inline=False)
    embed.add_field(name="👤 MEMBRO - 100", value=membro, inline=False)
    
    try:
        await ctx.author.send(embed=embed)
    except:
        await ctx.send("❌ Ativa sua DM")
